    ...
    public GuiItemStack generate()
    {
        ...
        GuiItemStack item = SimpleComponentBuilder.build(icon, name, 
            (e) -> {
                ...
                // Update item
                SurfaceGui.getOpenGui(e.getWhoClicked()).unwrap(SurfaceGui.class).rename("Skills | " + points + " skill " + PrintUtils.plural(points, "point", "points"));
                SurfaceGui.getOpenGui(e.getWhoClicked()).unwrap(SurfaceGui.class).updateSingleComponent(e.getWhoClicked(), e.getSlot(), generate());
            }, 
